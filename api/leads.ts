...
export default async function handler(req: any, res: any) {
  try {
    ...
    try {
      ...
      return fail(res, "SUPABASE_INIT_FAILED", 500, {
    ...
    if (req.method === "GET") {
      ...
      if (error) {
        return fail(res, "LEADS_FETCH_FAILED", 500, { details: error });
      }
    ...
    if (req.method === "POST") {
      ...
      if (error) {
        return fail(res, "LEAD_INSERT_FAILED", 500, { details: error });
      }
    ...
    // ✅ nunca mais “Function crashed” sem detalhe
