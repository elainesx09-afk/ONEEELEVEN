...
Acesse https://supabase.com/dashboard/project/cpuaflrmrxfovhqvljrw/sql/new e rode, **nesta ordem**:

1. `supabase/migrations/0050_agent_memory_system.sql` — sistema de memória cognitiva
   - Confirma: `CREATE EXTENSION` (pgvector), 5 tabelas `agent_memory_*`, 3 funções RPC, `get_unenriched_leads`
2. `supabase/migrations/0051_plan_limits.sql` — catálogo de planos + limites de uso
   - Confirma: tabela `plan_definitions` com 4 planos, funções `get_workspace_usage` e `check_workspace_limit`

## 🔴 PASSO 1.5 — Resolver o erro `INVALID_TOKEN_OR_WORKSPACE` (2 minutos)

A página **Leads** (e várias outras) mostra esse erro porque não existe um workspace + token válido. Conserta assim:

1. No SQL Editor do Supabase, rode `supabase/seed_pilot_workspace.sql`
2. Ele cria workspace + client + token + assinatura piloto, e **MOSTRA no resultado** dois valores:
   - `VITE_WORKSPACE_ID`
   - `VITE_API_TOKEN`
3. Copie esses dois valores — você vai usá-los no PASSO 2.
...
VITE_API_TOKEN              = <valor do seed — PASSO 1.5>
VITE_WORKSPACE_ID           = <valor do seed — PASSO 1.5>
...
## Auditoria das 11 páginas — status backend (sessão 2026-05-19)

**Descoberta importante:** todos os endpoints que o frontend chama JÁ EXISTEM no `api/`.
O erro `INVALID_TOKEN_OR_WORKSPACE` era o único bloqueador — o PASSO 1.5 (seed) resolve.

| # | Página | Endpoint | Status |
|---|---|---|---|
| 1 | Overview | `/api/overview` | ✅ existe — funciona após seed |
| 2 | Convertidos | `/api/leads` (filtro Fechado) | ✅ existe — funciona após seed |
| 3 | Leads | `/api/leads` | ✅ existe — funciona após seed |
| 4 | Pipeline | `/api/leads` (agrupado) | ✅ existe — funciona após seed |
| 5 | Caixa de Entrada | `/api/messages` + `/api/leads` | ✅ existe — funciona após seed |
| 6 | Follow-ups | `/api/leads/stale` | ✅ existe — funciona após seed |
| 7 | Configuração do Bot + KB | `/api/clients` + Supabase | ✅ existe — funciona após seed |
| 8 | Membros | `/api/workspaces` | ✅ existe — funciona após seed |
| 9 | Instâncias | `/api/instances` | ✅ existe — funciona após seed |
| 10 | Secretário | Supabase direto (tabela calls) | ⚠️ funciona se a tabela `calls` existir |
| 11 | MAESTRO | `/api/ai/brain` + cognitive | ✅ existe — IA depende de crédito Claude |

**Conclusão:** após PASSO 1 + 1.5 + 2, as páginas 1-9 funcionam. Página 11 (MAESTRO)
precisa de crédito Claude. Página 10 (Secretário) precisa da tabela `calls` no Supabase
(me avise se der erro nela que eu crio a migration).
